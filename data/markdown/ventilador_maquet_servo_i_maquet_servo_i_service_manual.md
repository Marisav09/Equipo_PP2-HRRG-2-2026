...
**Equipo:** Ventilador Servo-i Marquet
...
• O 2  cell, containing Pb (I)

• Air module, containing multiple PC boards (I)

• O 2  module, containing multiple PC boards (I)
...
CO 2  Analyzer module, Servo-i

• Servo-i – User's manual

• CO 2  Analyzer module, Servo-i – Installation Instructions
...
```metadata
pagina: 8
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P8_I0.png
contexto: 
```

![Imagen página 8 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P8_I0.png)

```metadata
pagina: 8
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P8_I1.png
contexto: 
```

![Imagen página 8 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P8_I1.png)
...
13. The gas modules, the O 2  cell and the safety valve pull magnet are connected to the PC 1780 Pneumatic back-plane.

```metadata
pagina: 10
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P10_I0.png
contexto: 
```

![Imagen página 10 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P10_I0.png)
...
```metadata
pagina: 10
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P10_I1.png
contexto: 
```

![Imagen página 10 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P10_I1.png)
...
14. Two gas modules, Air and O 2 , for regulation of the inspiratory gas.

15. Connector muff.

16. Inspiratory pipe with housings for the O 2  cell and for the safety valve.

17. O 2  cell incl. bacteria filter.

18. Safety valve.

19. Temperature sensor (inside the O 2  cell connector).
...
• Heating foil to keep a stable temperature in the expiratory gas.

• Pressure transducer connection, incl. bacteria filter, to connect the expiratory pressure transducer.
...
```metadata
pagina: 11
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P11_I1.png
contexto: 
```

![Imagen página 11 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P11_I1.png)
...
• Non-volatile memory. RAM with battery back-up. For settings, trends and logs. Present on PC 1771 and PC 1772.

• EEPROM. For PC board information, configuration, calibration data, etc. Present on almost all PC boards and in the O 2  cell.
...
The main block Inspiratory Section conveys the breathing gas from its gas inlets for Air and O 2  supply to the patient breathing system. It comprises the following main functions:

• Gas Modules – Air and O 2 .
...
Gas modules – Air and O 2 The Air and O 2  Gas Modules regulates the inspiratory gas flow and gas mixture.

SVX9003

1.Filter 2.Inspiratory valve temperature sensor 3.Supply pressure transducer 4.Flow transducer (Delta pressure transducer and net) 5.Nozzle unit with valve diaphragm 6.Inspiratory solenoid
...
Gas inlet Gas supply is connected to the ventilators gas inlet nipples. The design of the gas inlet nipples and their color markings vary according to the standard chosen.
...
Filter The Filter protects the ventilator from particles in the gas delivered to the Gas Modules. The filter must be replaced during the ”Preventive maintenance”.
...
Inspiratory valve temperature sensor The temperature of the supplied gas is measured by the Inspiratory Valve Temperature Sensor. This sensor is situated in the gas flow.

The output signal from this sensor is used to compensate for the gas density variations due to temperature.

Supply pressure transducer The pressure of the supplied gas is measured by the Supply Pressure Transducer.
...
Flow transducer The gas flows through a net (resistance) which causes a pressure drop. The pressure is measured on both sides of this net and the differential pressure value is then amplified.
...
Nozzle unit The plastic Nozzle Unit contains a valve diaphragm. The valve diaphragm, controlled by the Inspiratory Solenoid, regulates the gas flow through the Gas Module.
...
Inspiratory solenoid The gas flow through the Gas Module is regulated by the Inspiratory Solenoid via the Nozzle Unit.
...
Gas module key The Gas Modules are provided with a mechanical key to prevent that the module is mounted in the wrong slot.
...
• Housing and locking lever for the O 2  Cell with its bacteria filter.

• Housing for the Safety Valve.

• Connection for measurement of inspiratory pressure.

The pipe is provided with internal flanges with the purpose to improve mixing of O 2  and Air.

O 2  cell

The O 2  Cell is mounted in a housing on the Inspiratory Pipe and is protected by a bacteria filter.
...
The O 2  cell gives an output voltage proportional to the partial pressure of oxygen inside the Inspiratory pipe. At constant ambient pressure this output is proportional to the O 2  concentration in percent.

In each O 2  cell, the output signal will stay at a fairly constant level usually within 10–17 mV in normal air and at standard barometric pressure during the life time of the cell.

The life time of the cell is affected by the O 2 concentration. With a concentration (at the cell) in % and expected cell life time in hours the following applies at 25 o C (77 o F):

O 2  Conc. x Expected cell life = 500 000% hours.

The O 2  cell is automatically calibrated each time a Pre-use check is performed (if O 2  is connected to the ventilator).

If the ventilator has continually been in use for a long time, the measured O 2  concentration may drop due to normal degradation of the O 2  cell. This will activate a nuisance alarm. For further information, refer to the User's manual, chapter section 'O 2  cell adjustment'.

Note: Pre-use check is recommended to use to calibrate the O 2  cell.

An ID PROM is integrated into each O 2  cell. Its ID information and remaining lifetime can be read by the Servo-i.
...
A Temperature Sensor is integrated into the connector on top of the O 2  Cell. This sensor measures the temperature inside the Inspiratory Section.
...
• The pressure inside the inspiratory pipe is 5 cm H 2 O above the preset Upper Pressure Alarm limit. This condition is controlled by the Monitoring subsystem.

• The pressure inside the inspiratory pipe is 7 cm H 2 O above the preset Upper Pressure Alarm limit. This condition is controlled by the Breathing subsystem.

• The pressure inside the inspiratory pipe is above 117 ±7 cm H 2 O. This is an extra safety function and this situation will normally not occur.
...
The safety valve opening pressure is calibrated to 117 ±3 cm H 2 O during each Pre-use check.
...
Interconnecting board including connectors for cables to the Gas Modules as well as to the Safety Valve and to the O 2  Cell and the Temperature Sensor.
...
Expiratory inlet 22 mm / 10 mm tube connector for the expiratory tube of the patient breathing system. The inlet is designed to make condensed water drip out and allow use of a water trap for such water to be collected. Expiratory inlet bacteria filter can be connected to protect the cassette from contamination.

Heating foil An electrical Heating Foil applied on the outside of the expiratory pipe where the Ultrasonic Flowmeter is situated. The purpose of the Heating Foil to reduce condensation and maintain a stable temperature in the expiratory gas.

Ultrasonic flowmeter The Ultrasonic Flowmeter is a measuring device for the expiratory gas flow, using ultrasound technique with two ultrasonic transducers/recievers. The measuring process is controlled from the main block PC 1784 Expiratory Channel.
...
```metadata
pagina: 17
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P17_I0.png
contexto: 
```

![Imagen página 17 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P17_I0.png)

SVX9017

Expiratory pressure tube connector Bacteria filter Heating foil

PC 1786 Expiratory channel cassette Ultrasonic transducer Ultrasonic transducer
...
Bacteria filter and expiratory pressure tube Via a Bacteria Filter inside the cassette, the Expiratory Pressure Tube connects the cassette to the Expiratory Pressure Transducer. The filter and the connector are integrated parts of the cassette. The filter protects the transducer on PC 1781 Pressure Transducer from contamination.

Expiratory valve The Expiratory Valve consists of a membrane in the cassette that is operated by the axis of the Expiratory  Valve Coil. The valve is fully open as long as no power is supplied to the coil.
...
Expiratory valve coil The movable axis of the Expiratory Valve Coil controls the opening of the Expiratory Valve by pushing the valve membrane into desired position. The power supply to the coil is regulated so that the remaining pressure in the patient system, towards the end of the expiration time, is kept on the PEEP level according to front panel setting.

Expiratory outlet with expiratory one-way valve The gas from the patient system leaves the ventilator via this Expiratory Outlet. Backflow via the cassette is prevented by the Expiratory One-Way Valve. Its rubber membrane and valve seat are integrated parts of the Expiratory Outlet.

PC 1786 Expiratory channel cassette The PC 1786 Expiratory Channel Cassette is a connection board, integrated into the Expiratory Cassette, for the Ultrasonic Flowmeter and for the Heating Foil. It connects to PC 1785 mounted in the expiratory cassette compartment.
...
The pressure, conveyed via the pressure tube connected to this block, is led to and measured by its differential pressure transducer. With differential reference to the ambient pressure, the output signal is proportional to the measured pressure thus giving a linear measurement in the range -40 cm H 2 O to +160 cm H 2 O.

Technical limitation: Pressure exceeding ±400 cm H 2 O must be avoided.
...
5. Regulation of Inspiratory flow during inspiration time. The desired total Inspiratory flow value according to front panel settings is used to generate the flow reference signals Insp Flow Ref 1 and Insp Flow Ref 2. The level relation between these two flow reference signals depends on the desired O 2  concentration according to front panel setting. Insp Flow Ref 1 and Insp Flow Ref 2 are used for the control of its respective Gas Module (Air and O 2 ).
...
– The system is powered from a battery, but the battery voltage is too low for proper operation of the system.
...
• Internal fan using input signals from the Temperator sensor in the O 2  cell connector.
...
Connection slots for 6 optional modules, e. g. Battery modules or CO 2  Analyzer module.
...
The Internal Fan is controlled by the Temperature Sensor in the O 2  cell connector via electronics on PC 1775 Plug-and-play back-plane.
...
CO 2  Analyzer module (optional)

The CO 2  Analyzer module is an optional accessory that is connected to the Module unit.

The CO 2  Analyzer option allows for continuous monitoring shown in a waveform (capnogram) and as numericals on the screen.

The CO 2  Analyzer module is, via a cable, connected to a Capnostat sensor mounted on an airway adapter at the Y-piece. The sensor uses a solid state and IR based optical system with no moveable parts. It measures the difference between a reference light beam and one filtered for CO 2  wavelength.

The 'CO 2  Analyzer'- function must be enabled in the configuration software.
...
• Electrolube Bloc'Lube BLV15ML® on threads in contact with PC boards. • Loctite 243® on all other threads. Note : Threadlocking adhesive is not required on Heli-Coil® screw thread inserts as these screw thread-inserts have a self-locking function.
...
```metadata
pagina: 23
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P23_I0.png
contexto: 
```

![Imagen página 23 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P23_I0.png)
...
Note : Disassembling of the TFT Display must be performed in a clean and dustfree environment, as the TFT Display is sensitive to contaminants.
...
• Disconnect the four Direct access control- cables (9).
...
```metadata
pagina: 24
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I0.png
contexto: 
```

![Imagen página 24 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I0.png)
...
```metadata
pagina: 24
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I1.png
contexto: 
```

![Imagen página 24 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I1.png)
...
```metadata
pagina: 24
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I2.png
contexto: 
```

![Imagen página 24 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P24_I2.png)
...
• Remove the screws (14) holding the lamp. On older units, the lamps are mounted with a snap- in holder.
...
```metadata
pagina: 25
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I0.png
contexto: 
```

![Imagen página 25 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I0.png)

SVX9025

```metadata
pagina: 25
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I1.png
contexto: 
```

![Imagen página 25 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I1.png)

SVX9026

```metadata
pagina: 25
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I2.png
contexto: 
```

![Imagen página 25 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P25_I2.png)
...
– Remove the cover on each Direct access control- knob.

– Loosen the nut on each knob and pull off the knobs.

– Remove the nut holding each Direct access control and lift off the controls.
...
– One label strip for the Fixed keys on the upper right-hand area, and

– One label strip for the Special functions keys on the lower right-hand area of the User Interface.
...
```metadata
pagina: 26
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I0.png
contexto: 
```

![Imagen página 26 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I0.png)

```metadata
pagina: 26
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I1.png
contexto: 
```

![Imagen página 26 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I1.png)
...
```metadata
pagina: 26
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I2.png
contexto: 
```

![Imagen página 26 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I2.png)
...
```metadata
pagina: 26
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I3.png
contexto: 
```

![Imagen página 26 - 3](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P26_I3.png)
...
• While holding the label strip in correct position, remove the protective foils (3 and 4).
...
```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I0.png
contexto: 
```

![Imagen página 27 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I0.png)

```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I1.png
contexto: 
```

![Imagen página 27 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I1.png)
...
```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I2.png
contexto: 
```

![Imagen página 27 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I2.png)

```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I3.png
contexto: 
```

![Imagen página 27 - 3](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I3.png)
...
```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I4.png
contexto: 
```

![Imagen página 27 - 4](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I4.png)
...
```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I5.png
contexto: 
```

![Imagen página 27 - 5](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I5.png)

```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I6.png
contexto: 
```

![Imagen página 27 - 6](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I6.png)
...
```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I7.png
contexto: 
```

![Imagen página 27 - 7](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I7.png)

```metadata
pagina: 27
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I8.png
contexto: 
```

![Imagen página 27 - 8](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P27_I8.png)
...
```metadata
pagina: 28
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I0.png
contexto: 
```

![Imagen página 28 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I0.png)
...
```metadata
pagina: 28
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I1.png
contexto: 
```

![Imagen página 28 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I1.png)

SVX9042

```metadata
pagina: 28
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I2.png
contexto: 
```

![Imagen página 28 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I2.png)
...
```metadata
pagina: 28
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I3.png
contexto: 
```

![Imagen página 28 - 3](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P28_I3.png)
...
PC 1771, PC 1772 and PC 1784 • Remove the Patient Unit front cover.
...
– Disconnect PC 1789 cable connector (3). This is the cable for the optional PC 1789 Remote alarm connector.
...
Note : When replacing PC 1771 Control, PC 1772 Monitoring or PC 1784 Expiratory Channel, it can be necessary to re-install the System SW. For further information refer to section 'Replacing PC boards' in this chapter.
...
```metadata
pagina: 29
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I0.png
contexto: 
```

![Imagen página 29 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I0.png)

```metadata
pagina: 29
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I1.png
contexto: 
```

![Imagen página 29 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I1.png)
...
```metadata
pagina: 29
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I2.png
contexto: 
```

![Imagen página 29 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P29_I2.png)
...
Module unit including PC 1775 Plug-and- play back-plane
...
```metadata
pagina: 30
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I0.png
contexto: 
```

![Imagen página 30 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I0.png)
...
```metadata
pagina: 30
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I1.png
contexto: 
```

![Imagen página 30 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I1.png)

```metadata
pagina: 30
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I2.png
contexto: 
```

![Imagen página 30 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P30_I2.png)
...
```metadata
pagina: 31
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I0.png
contexto: 
```

![Imagen página 31 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I0.png)
...
```metadata
pagina: 31
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I1.png
contexto: 
```

![Imagen página 31 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I1.png)
...
```metadata
pagina: 31
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I2.png
contexto: 
```

![Imagen página 31 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I2.png)
...
```metadata
pagina: 31
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I3.png
contexto: 
```

![Imagen página 31 - 3](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P31_I3.png)
...
```metadata
pagina: 32
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I0.png
contexto: 
```

![Imagen página 32 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I0.png)
...
```metadata
pagina: 32
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I1.png
contexto: 
```

![Imagen página 32 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I1.png)
...
```metadata
pagina: 32
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I2.png
contexto: 
```

![Imagen página 32 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P32_I2.png)
...
Removal of the inspiratory channel, as well as replacement of O 2  cell/filter, is described in the User's manual, chapter 'Maintenance'.

```metadata
pagina: 33
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I0.png
contexto: 
```

![Imagen página 33 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I0.png)
...
```metadata
pagina: 33
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I1.png
contexto: 
```

![Imagen página 33 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I1.png)
...
• Carefully lift the mounting plate (2) and disconnect the O 2  cell connector and the safety valve pull magnet connector from PC 1780.
...
```metadata
pagina: 33
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I2.png
contexto: 
```

![Imagen página 33 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I2.png)

```metadata
pagina: 33
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I3.png
contexto: 
```

![Imagen página 33 - 3](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P33_I3.png)
...
Note 2 : The gas modules used in Servo-i are factory adjusted for this purpose. When replacing gas modules, make sure to use only Servo-i gas modules. Similar gas modules intended for the SV 300/300A or KION / KION-i Systems must not be used.

```metadata
pagina: 34
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I0.png
contexto: 
```

![Imagen página 34 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I0.png)
...
```metadata
pagina: 34
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I1.png
contexto: 
```

![Imagen página 34 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I1.png)

SVX9052

```metadata
pagina: 34
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I2.png
contexto: 
```

![Imagen página 34 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P34_I2.png)
...
```metadata
pagina: 35
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I0.png
contexto: 
```

![Imagen página 35 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I0.png)
...
```metadata
pagina: 35
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I1.png
contexto: 
```

![Imagen página 35 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I1.png)
...
```metadata
pagina: 35
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I2.png
contexto: 
```

![Imagen página 35 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P35_I2.png)
...
Operating capacity for the membrane is estimated to 10.000.000 breathing cycles. When this limit is passed or if the membrane for some reason has become defective, it must be replaced.
...
```metadata
pagina: 36
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I0.png
contexto: 
```

![Imagen página 36 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I0.png)

SVX9057

```metadata
pagina: 36
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I1.png
contexto: 
```

![Imagen página 36 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I1.png)
...
```metadata
pagina: 36
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I2.png
contexto: 
```

![Imagen página 36 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P36_I2.png)
...
```metadata
pagina: 37
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I0.png
contexto: 
```

![Imagen página 37 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I0.png)

SVX9059

```metadata
pagina: 37
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I1.png
contexto: 
```

![Imagen página 37 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I1.png)

SVX9060

```metadata
pagina: 37
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I2.png
contexto: 
```

![Imagen página 37 - 2](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P37_I2.png)
...
• Carefully find the correct position; the connector pins and the guides in the connectors must correspond. Do not turn the connector while inserting!
...
• Allow the Battery modules to charge until it is fully charged. To display battery status:

– Press the battery power symbol button on the battery. All four LEDs must be lit.

or

– Select Status / General on the User Interface. Check that Battery backup time is more than 30 minutes on the concerned battery.
...
• Let the Servo-i run in battery operation and check that the ' No battery capacity '-alarm is not activated within the specified time. The operating time is dependent on the number of Battery modules connected, refer to the 'Servo-i Ventilator System – User's manual'.

• Check that the time between the ' Limited battery capacity' - alarm and the ' No battery capacity '- alarm is more than 7 minutes.
...
```metadata
pagina: 40
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P40_I0.png
contexto: 
```

![Imagen página 40 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P40_I0.png)
...
Note : Do not remove the cable connector (1). Information stored in the PC board memory will be erased if the connector is removed:

– If the battery on PC 1771 is disconnected; user default configurations made via the Field Service System (FSS) and Pre-use check results including transducer calibrations will be erased.

– If the battery on PC 1772 is disconnected; all trends, all logs and Pre-use check results including transducer calibrations will be erased.
...
O 2  cell adaptation Measured O 2  concentration will be adapted in relation to set value.

Display Event logs.

Possible selections: • Latest logs • Selected time interval

Display Service logs.

Possible selections: • 21 latest logs • Selected time interval
...
• End Tidal CO 2  – Lower and Upper alarm limit
...
Adjustment of the O 2  concentration reading.

Refer to the 'Servo-i Ventilator System – User's Manual', chapter Operating your Servo-i / Operating - Adjustments for further information.
...
– Installed System SW ("see 'System SW version' in the General / Status window)
...
• The Service card is personal and must not be handed-over to anyone else.
...
During system start- up. Internal technical tests:
...
1. Restart the unit. Do not touch the User Interface during system start- up. Interfering with the knobs, keys, touch screen, loudspeaker grid, etc, may affect the internal technical tests.
...
•  If the Monitoring test failed: Replace PC 1772 Monitoring.

•  If the Panel test failed: Replace the loudspeaker or PC 1777 Panel.
...
Internal tests (continued)

If the Alarm output connector test failed:
...
Barometer test Checks that the barometric pressure measured by the internal barometer is within 630–1080 hPa.
...
Gas supply pressure test Checks that the gas supply pressures (Air and O 2 ) measured by the internal gas supply pressure transducers are within 200–650 kPa (2.0–6.5 bar).

Checks that the measured supply gas pressure values differs less than 20 mbar between MON and BRE.

(BRE + MON)

1. Check that the connected gas supply pressure (Air and O 2 ) is within the specified range.

2. Start the unit in a ventilation mode and check the alarms:

•  If an Air supply pressure-alarm is activated, replace the Gas module Air.

•  If an O 2  supply pressure-alarm is activated, replace the Gas module O 2 .
...
Internal leakage test Checks the internal leakage, with test tube connected, using the inspiratory and expiratory pressure transducers.

Checks that the leakage at 80 cm H 2 O is max. 10 ml/min.

Checks that the measured pressure values differs less than 5 cm H 2 O between Insp. and Exp.
...
•  Check that the cassette is correctly seated in the cassette compartment.

•  If possible, replace the expiratory cassette and check if the new cassette is accepted by the Pre-use check.

•  If the new cassette was accepted by the Pre-use check, the fault was located to the cassette. To repair the old cassette, check that the expiratory valve membrane is clean and correctly seated in the cassette. Replace the membrane if required.

3. Check that the pressure transducer tubes/filters are correctly mounted.

4. Check the inspiratory section:

•  Check that the inspiratory pipe is correctly mounted in the inspiratory section.

•  Check that the safety valve membrane is clean and correctly seated in the inspiratory pipe.
...
Internal leakage test (continued) If message ‘Pressure Transducer difference > 5 cm H 2 O’ appears:
...
Revision 02 Service Manual 6 - 7

6

Test Test description * Recommended action if the test fails

Pressure transducer test Calibrates and checks the inspiratory and expiratory pressure transducers.

The new zero value for the pressure transducers may not differ more than ±6 cm H 2 O from factory calibration.
...
During this test, the different subsystems concerned are compared. The difference between the sub- systems must not be more than ±1 cm H 2 O at 60 cm H 2 O.
...
•  If the new cassette was accepted by the Pre-use check, the fault was located to the cassette. The fail with the old cassette may in this case be due to water collected at the pressure transducer filter inside the cassette. Dry the old cassette properly.
...
Safety valve test Checks and if necessary adjusts the opening pressure for the safety valve to 117 ±3 cm H 2 O.
...
O 2  cell test Calibrates and checks the O 2  cell at 21% O 2  and 100% O 2 .

Checks if the O 2  cell is worn out.

As different gas mixtures are used during this test, calibration and check of O 2  cell will not be performed if one gas is missing.

(BRE + MON)

1. Check that the connected gas supply pressure (Air and O 2 ) is within the specified range.

2. Replace the O 2  cell.
...
Revision 02 Service Manual 6 - 9

6

Test Test description * Recommended action if the test fails

Flow transducer test Checks the inspiratory flow transducer. Calibrates and checks the expiratory flow transducer.

Calibrates at 60% O 2  and checks at 100% and 21% O 2 . As different gas mixtures are used during this test, calibration of the expiratory flow transducer will only be performed if both gases are connected. The check using the connected gas, (100% alt. 21% O 2 ) will however be performed. The ‘Flow transducer test’ will pass if the result of this check corresponds to the old calibration factor from a previous Pre-use check. The same expiratory cassette must be used.
...
Battery switch test Checks that the power supply switches to battery when mains power is disconnected.

Checks that the power supply switches back to mains power when re- connected.

This test will not be performed if:

•  Less than 10 min. backup time remains in the connected Battery module(s).
...
Patient circuit leakage test Checks the patient circuit leakage, with patient tubing connected, using the inspiratory and expiratory pressure transducers.

Checks that the leakage at 50 cm H 2 O is max. 80 ml/min.
...
Alarm state test Checks that no Technical error alarms are active during the Pre-use check.
...
Revision 02 Service Manual 6 - 11
...
1 POWER_FAILURE_MINUS_12_VOLTS_ TOO_LOW (i.e. < -13.2 V) 1. Replace PC 1778 DC/DC & Standard Connectors.

2 POWER_FAILURE_MINUS_12_VOLTS_ TOO_HIGH (i.e. > -10.8 V) 1. Check status of external battery (if connected).

2. Replace PC 1778 DC/DC & Standard Connectors.

3. Replace PC 1775 Plug-and-play back-plane.

3 POWER_FAILURE_12_VOLTS_TOO_LOW 1. Check status of external battery (if connected).

2. Replace PC 1778 DC/DC & Standard Connectors.

3. Replace PC 1775 Plug-and-play back-plane.

4 POWER_FAILURE_12_VOLTS_TOO_HIGH 1. Replace PC 1778 DC/DC & Standard Connectors.

5 POWER_FAILURE_24_VOLTS_TOO_LOW 1. Replace PC 1778 DC/DC & Standard Connectors.

2. Replace the gas modules. Replace one gas module at a time and check that this technical error code will not appear.

6 POWER_FAILURE_24_VOLTS_TOO_HIGH 1. Replace PC 1778 DC/DC & Standard Connectors.
...
8 TECH_ERR_INSP_PAUSE_HOLD_TIME_ EXCEEDED 1. Replace PC 1771 Control.

9 TECH_ERR_EXP_PAUSE_HOLD_TIME_ EXCEEDED 1. Replace PC 1771 Control.
...
Revision 02 Service Manual 6 - 13
...
35 EXP_FLOW_MTR_NODE_CONNECT_TIME- OUT 1. Replace PC 1784 Expiratory channel.
...
41 REAL_TIME_CLOCK_ERR 1. Check that System SW V1.02.05 (or later) is installed. Update System SW if required.
...
Revision 02 Service Manual 6 - 15
...
20003 PANEL_BUTTON_STUCK 1. Restart the unit. Do not touch the User Interface during system start-up. Interfering the knobs, keys, touch screen, loudspeaker grid, etc, may affect the internal technical tests.
...
20004 PANEL_AUDIO_FAILED 1. Restart the unit. Do not touch the User Interface during system start-up. Interfering the knobs, keys, touch screen, loudspeaker grid, etc, may affect the internal technical tests.
...
Revision 02 Service Manual 6 - 17
...
C. Bacteria filter for the inspiratory pressure transducer

D. Bacteria filter for the O 2  cell
...
A. Replace filters in the gas modules. When replacing filter, move the rubber seal from the old to the new filter.
...
```metadata
pagina: 61
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P61_I1.png
contexto: 
```

![Imagen página 61 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P61_I1.png)
...
C. Replace the filter for the inspiratory pressure transducer. Make sure that the filter is correctly seated into the rubber ring.
...
Bacteria filter for O 2  cell

D. Replace the bacteria filter for the O 2  cell.

Refer to instructions in the 'Servo-i Ventilator System – User's manual', chapter 'Maintenance'.

```metadata
pagina: 62
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P62_I0.png
contexto: 
```

![Imagen página 62 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P62_I0.png)
...
```metadata
pagina: 62
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P62_I1.png
contexto: 
```

![Imagen página 62 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P62_I1.png)
...
• Lower the locking catch (1) and disconnect the connector (2). This is done to protect the O 2  cell during the Internal fan-test.
...
```metadata
pagina: 63
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P63_I0.png
contexto: 
```

![Imagen página 63 - 0](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P63_I0.png)
...
```metadata
pagina: 63
imagen: data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P63_I1.png
contexto: 
```

![Imagen página 63 - 1](data/images/ventilador_maquet_servo_i_maquet_servo_i_service_manual/P63_I1.png)
...
– Brightness (backlight). Estimated lifetime (with acceptable brightness level) for the lamps is 30,000 hours. Using the Field Service System (FSS), a time meter for the lamps can be shown. The time meter must be reset after replacement of the lamps.
...
– Check the touch screen functions. Press buttons on different parts of the touch screen.

– Check the Main Rotary Dial. Turn and press the Main Rotary Dial.

– Check the Direct Access Knobs. Turn and press all Direct Access Knobs.

– Check the membrane buttons. Press all membrane buttons.
...
• Connect the gas supplies (Air and O 2 ).
...
– Check the manufacturing date of the Battery modules. Manufacturing date (year-week) is printed on the battery label. The batteries must be replaced after 3 years.

– Check that the battery housing is not damaged.

– Press the battery power symbol button and check that the green LEDs are lit. All 4 LEDs must be lit if the battery is fully charged.
...
EXPIRATORY OUTLET ULTRASONIC FLOWMETER
...
PC1775 PLUG- AND- PLAY BACK- PLANE
...
SAFETY VALVE SAFETY VALVE PULL MAGNET

PC1780 PNEUMATIC BACK-PLANE C C M

C M

EXPIRATORY CHANNEL CHANNEL CONNECTOR C

INSP. PRESSURE TRANSDUCER EXP. PRESSURE

TRANSDUCER 781 PC1781 PC PC 1781

X1

OPTIONAL MODULES (e. g. BATTERY and Future options)
...
CONNECTOR INCLUDING CAN BUS

COOLING AIR FLOW

MICRO PROCESSOR

ID PROM COMMUNICATING WITH MONITORING ID
...
PC1789 REMOTE ALARM CONNECTOR

P67 ALARM OUTPUT P66
